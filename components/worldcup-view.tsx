...
import { STAGES, buildGroups, type Match } from "@/lib/worldcup";
import { analyzeGroups } from "@/lib/qualification";
import { resolveBracket, type ResolvedMatch } from "@/lib/bracket";
...
export function WorldCupView({ initialMatches }: { initialMatches: Match[] }) {
  ...
  // Cada pestaña es su propia vista, así que al cambiar volvemos arriba.
  useEffect(() => {
    window.scrollTo(0, 0);
  }, [tab]);

  // Una sola pasada de standings alimenta los escenarios de grupo y el bracket.
  const groups = useMemo(() => buildGroups(matches), [matches]);
  const scenarios = useMemo(
    () => analyzeGroups(matches, groups),
    [matches, groups],
  );
  const bracket = useMemo(
    () => resolveBracket(matches, groups),
    [matches, groups],
  );
    ...
    () =>
      new Set(
        bracket.thirds
          ...
          .map((t) => `Group ${t.letter}`),
      ),
    [bracket],
  );

  // La pestaña final junta semifinales y final; hasta que se resuelva algún
  // equipo muestra el placeholder en vez de "Ganador #97 vs Ganador #98".
  const knockout = useMemo(() => {
    const fin = [...bracket.byStage.SF, ...bracket.byStage.F];
    const finKnown = fin.some((m) => m.team1.team || m.team2.team);
    return {
      R32: bracket.byStage.R32,
      R16: bracket.byStage.R16,
      QF: bracket.byStage.QF,
      FIN: finKnown ? fin : [],
    };
  }, [bracket]);
    ...
    >
      ...
      >
        ...
        >
          ...
          {/* Eliminatorias (placeholders resueltos por el motor) */}
